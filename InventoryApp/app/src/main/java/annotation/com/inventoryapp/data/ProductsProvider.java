...
public class ProductsProvider extends ContentProvider {
    ...
    private Uri insertPet(Uri uri, ContentValues values) {
        ...
        String price = values.getAsString(ProductsContract.ProductEntry.COLUMN_PRICE);
        if (price == null) {
            throw new IllegalArgumentException("Price is required");
        }

        String quantity = values.getAsString(ProductsContract.ProductEntry.COLUMN_QUANTITY);
        if (quantity == null) {
            throw new IllegalArgumentException("Quantity is required");
        }

        String image = values.getAsString(ProductsContract.ProductEntry.COLUMN_IMAGE);
        if (image == null) {
    ...
    public int update(Uri uri, ContentValues contentValues, String selection,
        ...
        switch (match) {
            case PRODUCTS:
                return updateProoduct(uri, contentValues, selection, selectionArgs);
            case PRODUCT_ID:
    ...
    private int updateProoduct(Uri uri, ContentValues values, String selection, String[] selectionArgs) {
        ...
        if (values.containsKey(ProductsContract.ProductEntry.COLUMN_PRICE)) {
            Integer price = values.getAsInteger(ProductsContract.ProductEntry.COLUMN_PRICE);
            if (price == null) {
                throw new IllegalArgumentException("Price is required");
            }
        }
        ...
        if (values.containsKey(ProductsContract.ProductEntry.COLUMN_QUANTITY)) {
            Integer quantity = values.getAsInteger(ProductsContract.ProductEntry.COLUMN_QUANTITY);
            if (quantity == null) {
                throw new IllegalArgumentException("quantity is required");
            }
        }

        if (values.containsKey(ProductsContract.ProductEntry.COLUMN_IMAGE)) {
            Integer image = values.getAsInteger(ProductsContract.ProductEntry.COLUMN_IMAGE);
            if (image == null) {
                throw new IllegalArgumentException("image is required");
            }
        }
